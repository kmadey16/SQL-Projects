...
SELECT DISTINCT f.name AS Facility, CONCAT(firstname," ",surname) AS name 
FROM Members
LEFT JOIN Bookings
...
(CASE WHEN memid >0
THEN f.membercost * SUM(slots)
ELSE f.guestcost * SUM(slots)
END) AS cost
FROM Bookings AS b
...
SELECT Facility, CONCAT(m.firstname, " ", m.surname) AS Member,
(CASE WHEN m.memid > 0 THEN membercost * b.slots
ELSE guestcost * b.slots
END) AS cost
FROM Members as m,
	(SELECT 
	SUM(slots) AS slots, memid, facid 
	FROM Bookings
	WHERE starttime LIKE '2012-09-14%'
	GROUP BY facid, memid) AS b,
	(SELECT name AS Facility, facid, membercost, guestcost FROM Facilities) AS f
